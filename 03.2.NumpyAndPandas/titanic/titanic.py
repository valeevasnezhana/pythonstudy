...
def is_lucky(ticket: str) -> bool:
    if ticket.isdecimal() and len(ticket) % 2 == 0:
        center_right = len(ticket) // 2
        sum_of_firsts = sum(map(int, ticket[:center_right]))
        sum_of_seconds = sum(map(int, ticket[center_right:]))
        return sum_of_firsts == sum_of_seconds
    return False
...
def dead_lucky(df: pd.DataFrame) -> float:
    ...
    """
    is_lucky_series = df["Ticket"].apply(is_lucky)
    return 1 - df[is_lucky_series]["Survived"].mean()
