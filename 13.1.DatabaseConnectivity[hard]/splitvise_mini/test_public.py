...
def _make_money(dct: tp.Mapping[int, tp.Union[int, float]]) -> tp.Dict[int, Decimal]:
    return {key: Decimal(value) for key, value in dct.items()}


def _to_user_id(dct: tp.Mapping[str, tp.Any],
                username_to_id: tp.Mapping[tp.Optional[str], int]) -> tp.Dict[int, tp.Any]:
...
def _create_default_users() -> dict[tp.Optional[str], int]:
    with Session.begin() as session:
        users = [User(username=username) for username in DEFAULT_USERNAMES]
        session.add_all(users)
        session.flush()
        default_users_to_id: tp.Dict[tp.Optional[str], int] = {str(u.username): int(u.user_id) for u in users}
